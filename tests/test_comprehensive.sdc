...
# 注意：-max 和 -min 是纯标志，不能同时指定两个延迟值
# 应该分别调用两次或使用 -add_delay
...
# group_path -name critical -weight 5 -from [get_pins start] -to [get_pins end]
# group_path -name io_paths -weight 2 -from [all_inputs] -to [all_outputs]

# -----------------------------------------------------------------------------
# 14. 对象访问命令（在约束中使用）
# -----------------------------------------------------------------------------

# 这些命令通常在其他约束中使用，但也可以单独使用
set all_clocks_list [all_clocks]
set all_inputs_list [all_inputs]
set all_outputs_list [all_outputs]
set all_regs_list [all_registers]

# 带过滤的对象访问
set input_ports [all_inputs -no_clocks]
set clocked_regs [all_registers -clocked_by clk]
...
set_timing_derate -early 0.98 -late 1.02 -cell_delay
